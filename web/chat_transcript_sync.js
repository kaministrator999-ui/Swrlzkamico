...
function sessionFor(context){
  ...
  if(!s){s={requestId:rid,context,raw:'',networkSeq:0,identity:null,syncing:false,pending:[],syncCount:0,lastSyncAt:0,contractFailed:false};sessions.set(rid,s)}
  s.context=context;
  return s;
}
...
async function syncSession(s,reason){
  ...
  try{
    ...
    const m=messageFor(s);if(m){m.meta=m.meta||{};m.meta.transcriptSync={contract:TRANSCRIPT_CONTRACT,state:snapshot.terminal?'terminal-synced':'live-synced',strict:true,requiredServerVersion:REQUIRED_SERVER_VERSION,networkSeq:s.networkSeq,rawChars:s.raw.length,textRevision:Number(snapshot.textRevision||0),phase:String(snapshot.phase||''),syncCount:s.syncCount,lastSyncAt:s.lastSyncAt,reason:String(reason||'foreground')}}
...
window.__swrlzTranscriptSync={version:2,contract:TRANSCRIPT_CONTRACT,requiredServerVersion:REQUIRED_SERVER_VERSION,strict:true,policy:'server-transcript-required-no-fallback',syncActive};
})();
