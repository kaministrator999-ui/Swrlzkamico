...
function cognitiveClock(prompt){
  ...
  const identity=/\b(?:what(?:'s| is) your name|who are you|your identity|what are you called)\b/i.test(lower);
  ...
  const referenceFrame=roles.includes('social-affect')?'conversational':domains[0]?.domain==='programming'?'technical':resolutionDepth==='deep'?'analytical':'general';
  return {architecture:'RMCCA',version:1,diagnosticHeuristic:true,structuralRoles:roles,domains,resolutionDepth,referenceFrame,responseTopology,synthesisOrder:domains.map(d=>d.domain)};
}
function annotateRequest(payload,diag,clock){
  try{
    ...
    message.meta.contextCamera={...(message.meta.contextCamera||{}),turnIntent:String(payload.turnIntent||'unknown'),historySource:diag.source,historyMessages:diag.history.length,assistantHistoryUsingModelText:diag.canonicalCount,promptChars:String(payload.prompt||'').length,directiveId:DIRECTIVE_ID,cognitiveClock:clock};
  }catch(_){ }
}
...
window.fetch=function(input,init={}){
  ...
  const diag=canonicalHistory(payload),clock=cognitiveClock(payload.prompt);
  const next={...payload,responseDirective:DIRECTIVE,history:diag.history};
  annotateRequest(next,diag,clock);
  return priorFetch(input,{...init,body:JSON.stringify(next)});
};

window.__swrlzCanonicalContext={directiveId:DIRECTIVE_ID,canonicalHistory,cognitiveClock};
})();
