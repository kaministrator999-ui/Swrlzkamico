...
function canonicalizePayload(payload){
  const normalized=fallbackNormalizePayload(payload);let prepared=normalized,error='';
  try{const prepare=window.__swrlzCanonicalContext?.preparePayload;if(typeof prepare==='function')prepared=prepare(normalized)}catch(err){error=String(err?.message||err)}
  try{
    const ctx=window.__swrlzCanonicalContext;
    if(ctx&&typeof ctx.envelopeValid==='function'&&!ctx.envelopeValid(prepared)&&typeof ctx.preparePayload==='function')prepared=ctx.preparePayload({...normalized,swrlzCognitiveContext:null});
  }catch(err){error=[error,String(err?.message||err)].filter(Boolean).join(' | ')}
  if(prepared&&typeof prepared==='object'&&error)prepared={...prepared,swrlzCanonicalTransportError:error};
  return prepared;
}
...
async function captureOriginInstance(rid){const instanceId=await serverInstanceId();if(!instanceId)return;const store=loadStore();const entry=store[rid];if(!entry)return;if(!entry.originInstanceId)entry.originInstanceId=instanceId;entry.lastSeenInstanceId=instanceId;entry.updatedAt=Date.now();saveStore(store)}
function remember(payload,url){
  if(!payload?.requestId)return;
  const store=prune(),rid=String(payload.requestId),old=store[rid]||{},stamp=Date.now();
  const branchCounter=Math.max(1,Number(old.branchCounter||0)||1);
  store[rid]={...old,requestId:rid,threadId:String(payload.threadId||''),url:String(url||'/api/chat?action=stream'),body:payload,createdAt:Number(old.createdAt||stamp),requestStartedAt:Number(old.requestStartedAt||stamp),updatedAt:stamp,recoveryAttempts:Number(old.recoveryAttempts||0),branchCounter};
  saveStore(store);
  const message=messageForRequest(rid);applyTransportCamera(message,payload,{generationBranchId:`${rid}:branch-${branchCounter}`,requestAttempt:1,firstSendReceiptAttached:Boolean(payload?.swrlzCognitiveContext)});
  captureOriginInstance(rid);
}
...
function currentContext(rid,threadId){try{const thread=typeof currentThread==='function'?currentThread():null;if(!thread)return null;if(threadId&&String(thread.id||'')!==String(threadId))return null;const messages=Array.isArray(thread.messages)?thread.messages:[];const message=[...messages].reverse().find(item=>item?.role==='assistant'&&String(item?.meta?.requestId||'')===String(rid));return message?{thread,message}:null}catch(_){return null}}
function phaseNote(message,text,phase='RECONNECTING'){if(!message)return;message.meta=message.meta||{};message.meta.phase=phase;message.meta.error='';message.meta.trail=Array.isArray(message.meta.trail)?message.meta.trail:[];const last=message.meta.trail[message.meta.trail.length-1];if(last?.reason!==text)message.meta.trail.push({seq:Number(last?.seq||0)+1,phase,reason:text,at:Date.now()});if(message.meta.trail.length>80)message.meta.trail.splice(0,message.meta.trail.length-80);if(message.state!=='complete')message.state='streaming';try{saveState()}catch(_){ }try{scheduleRender(false)}catch(_){ }}
...
function markRecoveryCamera(message,entry,body,branchId){if(!message)return;applyTransportCamera(message,body,{recoveryAttempts:Number(entry?.recoveryAttempts||0),requestStartedAt:Number(entry?.requestStartedAt||entry?.createdAt||0),recoveryElapsedMs:entry?.requestStartedAt?Math.max(0,Date.now()-Number(entry.requestStartedAt)):null,generationBranchId:branchId,requestAttempt:Number(entry?.recoveryAttempts||0)+1})}
async function compareInstance(entry,message){const current=await serverInstanceId();if(!current)return;const store=loadStore(),saved=store[entry.requestId];if(saved){saved.lastSeenInstanceId=current;saved.updatedAt=Date.now();saveStore(store)}const previous=String(entry.lastSeenInstanceId||entry.originInstanceId||'');if(previous&&previous!==current){message.meta=message.meta||{};message.meta.backgroundInstanceChanged=true;phaseNote(message,'Server instance changed. The same canonical request is being resumed or regenerated and reconciled against the partial response already on this device.')}}
...
function replayDelta(event,context){
  ...
  reconcile.mode=mode;message.text=target;message.meta=message.meta||{};message.meta.backgroundResumeMode=mode;message.meta.generationBranchId=context.__swrlzGenerationBranchId;
...
async function resumeOne(rid){
  ...
  entry.recoveryAttempts=Number(entry.recoveryAttempts||0)+1;entry.branchCounter=Math.max(1,Number(entry.branchCounter||1))+1;entry.updatedAt=Date.now();saveStore(store);
  const body=canonicalizePayload(entry.body);entry.body=body;saveStore(store);
  const branchId=`${rid}:branch-${entry.branchCounter}`;markRecoveryCamera(found.message,entry,body,branchId);
  const context={threadId:found.thread.id,requestId:rid,message:found.message,lastSeq:0,terminal:false,started:performance.now(),requestStartedAt:Number(entry.requestStartedAt||entry.createdAt||Date.now()),__swrlzReplay:true,__swrlzGenerationBranchId:branchId,__swrlzReconcile:{baseline:String(found.message.text||entry.partialText||''),generated:'',mode:'compare',redoNoted:false}};
  live.add(rid);phaseNote(found.message,'Checking the pending canonical server generation now. Existing work will continue if available; otherwise the exact same cognitive request will restart and reconcile against the saved partial response already on this device.');
  await compareInstance(entry,found.message);
  try{const allHeaders={...chatAuthHeaders(),'Accept':'application/x-ndjson'};const response=await nativeFetch(entry.url||'/api/chat?action=stream',{method:'POST',credentials:'same-origin',cache:'no-store',headers:allHeaders,body:JSON.stringify(body)});if(response.status===401){phaseNote(found.message,'Generation recovery needs a valid Chat access token before it can reconnect.','AUTH_REQUIRED');return}await consumeNdjson(response,context,rid)}catch(err){const text=String(err?.message||err);phaseNote(found.message,`Generation recovery is retrying (${text}). The pending canonical request remains saved on this device.`);if(document.visibilityState!=='hidden')scheduleRetry(rid)}finally{live.delete(rid)}
}
