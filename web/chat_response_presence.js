...
function setPresence(message,label,phase='ANALYZING_REQUEST',stage='waiting'){if(!message||!['streaming','cancelling'].includes(String(message.state||'')))return;message.meta=message.meta||{};message.meta.phase=phase;message.meta.workLabel=normalizeBrandText(label);message.meta.responsePresence={...(message.meta.responsePresence||{}),version:1,stage,label:message.meta.workLabel,at:Date.now()};renderNow(message)}
...
function armInitial(rid){const message=currentMessage(rid);if(!message)return;clearTimers(rid);setPresence(message,`${BRAND} received your message…`,'ANALYZING_REQUEST','received');const list=[];list.push(setTimeout(()=>{const m=currentMessage(rid);if(m?.meta?.responsePresence?.stage==='received')setPresence(m,`${BRAND} is reading your message…`,'ANALYZING_REQUEST','reading')},320));list.push(setTimeout(()=>{const m=currentMessage(rid);if(['received','reading'].includes(String(m?.meta?.responsePresence?.stage||'')))setPresence(m,`${BRAND} is understanding your request…`,'ANALYZING_REQUEST','understanding')},1050));timers.set(String(rid),list)}
function labelFor(event,message){const p=String(event?.phase||'').toUpperCase(),t=String(event?.type||'').toUpperCase();
  ...
  if(t==='FAILED'||p==='ERROR')return ['Request failed','ERROR','failed'];
  if(t==='CANCELLED'||p==='CANCELLED')return ['Request cancelled','CANCELLED','cancelled'];
  if(p==='RECONNECTING')return [`${BRAND} is reconnecting to this response…`,'RECONNECTING','reconnecting'];
  ...
  if(p==='PREFILL')return [`${BRAND} is preparing the conversation context…`,'PREFILL','context'];
  if(p==='COMPUTE_HEARTBEAT'){
    const prior=String(message?.meta?.responsePresence?.stage||'');
    return [prior==='context'?`${BRAND} is thinking through the context…`:`${BRAND} is thinking about your response…`,'ANALYZING_REQUEST','thinking'];
  }
  if(['COGNITIVE_ROUTE','ROUTE_RESOLVED','RESPONSE_PLAN','ANALYZING_REQUEST','CAPABILITY_DISCOVERY','PERMISSION_PREFLIGHT'].includes(p)||t==='STARTED'||t==='ROUTE')return [`${BRAND} is thinking about your response…`,'ANALYZING_REQUEST','thinking'];
  if(['FIRST_TOKEN','GENERATING','DECODE_PROGRESS','ANSWER_STREAMING'].includes(p))return [`${BRAND} is composing your response…`,'GENERATING','composing'];
  if(t==='DELTA')return [`${BRAND} is writing your response…`,'GENERATING','writing'];
...
window.__swrlzResponsePresence={version:1,brand:BRAND,aliases:['swurlz','swrlz','swyrlz'],normalizeBrandText,labelFor,policy:'stage-aware-response-presence-display-only-brand-normalization'};
})();
