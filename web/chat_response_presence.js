...
function applyVisibleLabel(message){try{if(!message?.id||!message?.meta?.workLabel)return;const article=document.querySelector(`.message[data-message-id="${CSS.escape(String(message.id))}"]`);const target=article?.querySelector('.empty-response span:last-child');if(target&&target.textContent!==message.meta.workLabel)target.textContent=message.meta.workLabel}catch(_){}}
function repairVisiblePresence(){try{const thread=typeof currentThread==='function'?currentThread():null;for(const message of thread?.messages||[]){if(message?.role!=='assistant'||!['streaming','cancelling'].includes(String(message.state||'')))continue;if(String(message.meta?.phase||'').toUpperCase()==='CONNECTING'&&!message.meta?.workLabel){message.meta=message.meta||{};message.meta.phase='ANALYZING_REQUEST';message.meta.workLabel=`📥 ${BRAND} is receiving your message…`;message.meta.responsePresence={...(message.meta.responsePresence||{}),version:3,stage:'receiving',label:message.meta.workLabel,at:Date.now()}}applyVisibleLabel(message)}}catch(_){}}
function renderNow(message){try{if(typeof saveState==='function')saveState()}catch(_){ }try{if(typeof scheduleRender==='function')scheduleRender(false)}catch(_){ }try{if(typeof refs!=='undefined'&&refs?.composerHint&&message?.meta?.workLabel)refs.composerHint.textContent=message.meta.workLabel}catch(_){ }requestAnimationFrame(()=>{applyVisibleLabel(message);repairVisiblePresence()})}
function setPresence(message,label,phase='ANALYZING_REQUEST',stage='waiting'){if(!message||!['streaming','cancelling'].includes(String(message.state||'')))return;message.meta=message.meta||{};message.meta.phase=phase;message.meta.workLabel=normalizeBrandText(label);message.meta.responsePresence={...(message.meta.responsePresence||{}),version:3,stage,label:message.meta.workLabel,at:Date.now()};renderNow(message)}
...
function armInitial(rid,attempt=0){
  const message=currentMessage(rid);if(!message){if(attempt<6)setTimeout(()=>armInitial(rid,attempt+1),Math.min(80,8*(attempt+1)));return}clearTimers(rid);
...
let normalizationQueued=false;function queueNormalization(){if(normalizationQueued)return;normalizationQueued=true;queueMicrotask(()=>{normalizationQueued=false;normalizeTree(document.body);repairVisiblePresence()})}
...
function boot(){normalizeTree(document.body);repairVisiblePresence();observer.observe(document.body,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:['aria-label','title','placeholder']});requestAnimationFrame(repairVisiblePresence)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();

window.__swrlzResponsePresence={version:3,brand:BRAND,aliases:['swurlz','swrlz','swyrlz'],normalizeBrandText,labelFor,repairVisiblePresence,policy:'stage-aware-expressive-response-presence-owns-waiting-bubble-and-normalizes-brand'};
})();
