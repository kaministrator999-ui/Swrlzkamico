...
function send(e){const payload=JSON.stringify({runId:RUN,url:location.pathname,ua:navigator.userAgent,event:e});try{fetch(ENDPOINT,{method:'POST',headers:{'content-type':'application/json'},body:payload,keepalive:true,cache:'no-store',credentials:'same-origin'}).catch(()=>{})}catch(_){ }}
function beacon(e){const payload=JSON.stringify({runId:RUN,url:location.pathname,ua:navigator.userAgent,event:e});try{if(navigator.sendBeacon)navigator.sendBeacon(ENDPOINT,new Blob([payload],{type:'application/json'}))}catch(_){ }}
