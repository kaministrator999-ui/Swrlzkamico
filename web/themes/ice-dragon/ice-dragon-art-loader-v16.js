/* SWRLZ Ice Dragon asset hydrator v16 — instant local preview + persistent device cache + non-blocking 8K promotion */
(()=>{'use strict';
...
const FAST_SOURCE_KEY='swrlz.theme.iceDragon.adultPreview.v1';
const FAST_COMPANION_KEY='swrlz.theme.iceDragon.companionPreview.v1';
...
async function seedFastPreview(blob,key,label,minChars){
  try{
    const existing=localStorage.getItem(key)||'';
    if(existing.startsWith('data:image/')&&existing.length>=minChars){debug.log('fast-preview-existing',`${label} chars=${existing.length}`);return true}
    const dataUrl=await blobDataUrl(blob);
    if(!dataUrl.startsWith('data:image/')||dataUrl.length<minChars)throw new Error(`${label} preview unexpectedly small`);
    localStorage.setItem(key,dataUrl);debug.log('fast-preview-store',`${label} chars=${dataUrl.length}`);return true;
  }catch(error){debug.log('fast-preview-store-failed',`${label} ${String(error)}`);return false}
}
...
async function ensureCompanion(){
  ...
  companionPromise=(async()=>{
    try{
      ...
      seedFastPreview(blob,FAST_COMPANION_KEY,'companion',1000);
...
async function sourceBlob(){
  ...
  if(cached){const blob=await cached.blob();debug.log('adult-cache-hit',`tier=864 bytes=${blob.size}`);seedFastPreview(blob,FAST_SOURCE_KEY,'adult-864',100000);return blob}
  ...
  await cachePut(SOURCE_KEY,blob);debug.log('adult-cache-store',`tier=864 bytes=${blob.size}`);seedFastPreview(blob,FAST_SOURCE_KEY,'adult-864',100000);return blob;
}
...
async function buildUltra(source){
  ...
  ultraBuildPromise=(async()=>{
    ...
    try{
      ...
      const blob=await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error('8K WebP encode failed')),'image/webp',0.82));
