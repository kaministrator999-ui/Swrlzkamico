/* SWRLZ Ice Dragon asset hydrator v14 — 864x1536 adult source + single-flight hydration */
(()=>{'use strict';
...
function ensureAdult(){
  ...
  adultPromise=(async()=>{
    try{
      debug.log('adult-part-fetch-start',`parts=${ADULT_PARTS.length}`);
      const parts=await Promise.all(ADULT_PARTS.map(read));
      const raw=parts.join('');
      debug.log('adult-parts-complete',`parts=${parts.length} rawChars=${raw.length}`);
      const adult=normalizeBase64(raw,'adult-864x1536');
      if(adultUrl)URL.revokeObjectURL(adultUrl);
      adultUrl=blobUrl(adult);debug.log('adult-blob-created',adultUrl.slice(0,48));
      const probe=new Image();
      await new Promise((resolve,reject)=>{probe.onload=()=>{debug.log('adult-decode-ok',`${probe.naturalWidth}x${probe.naturalHeight}`);if(probe.naturalWidth<800||probe.naturalHeight<1400)reject(new Error(`adult wallpaper resolution unexpectedly low ${probe.naturalWidth}x${probe.naturalHeight}`));else resolve()};probe.onerror=()=>reject(new Error('adult wallpaper JPEG failed browser decode'));probe.src=adultUrl});
