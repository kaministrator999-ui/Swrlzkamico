...
def _session_valid(value: str) -> bool:
    ...
    try:
        ...
        if not isinstance(payload, dict):
            return False
        if payload.get("v") != SESSION_VERSION or payload.get("kind") != "admin-chat-session":
            return False
        now = int(time.time())
...
def install(server) -> None:
    ...
    async def admin_chat_session(request: Request):
        ...
        try:
            ...
            return JSONResponse(
                {"ok": False, "code": str(exc), "detail": "Admin Chat session signing is unavailable."},
                status_code=503,
                headers=chat._no_store_headers(),
            )
        return JSONResponse(
            {"ok": True, "session": token, "expiresAt": expires_at, "ttlSeconds": SESSION_TTL_SECONDS, "credentialKind": "STATELESS_ADMIN_CHAT_SESSION"},
            headers=chat._no_store_headers(),
        )
