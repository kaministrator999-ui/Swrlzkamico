...
def file_info(path: Path) -> dict:
    ...
    return {"name": path.name, "path": str(path), "type": "directory" if path.is_dir() else "file", "size": stat.st_size, "modified": int(stat.st_mtime), "mime": mime, "editable": path.is_file() and text_candidate(path, mime) and stat.st_size <= MAX_TEXT_EDIT}
...
def run_gate5() -> dict:
    ...
    env = os.environ.copy()
    env.update({"SWRLZ_R39_PATH": str(RAW), "SWRLZ_LALM_PATH": str(RAW), "SWRLZ_R39_GZ_PATH": str(PACKED), "SWRLZ_LIVE_DIR": str(LIVE)})
    started = time.time()
    proc = subprocess.run([sys.executable, str(GATE5)], cwd=str(LIVE), env=env, capture_output=True, text=True, errors="replace", timeout=240)
...
</style></head><body><h1>§wyrlz SERVER Workbench <span class="badge">v2.0.5</span></h1><p class="muted">Unified Admin + R39 + Gate 5 runtime. Vercel /tmp remains ephemeral.</p>
...
async function mediaPreview(tag,j){let b=await api('download',{path:j.path},null,'blob'),url=URL.createObjectURL(b),v=document.getElementById('viewer');if(tag==='img')v.innerHTML=`<img src="${url}">`;else if(tag==='video')v.innerHTML=`<video controls src="${url}"></video>`;else if(tag==='audio')v.innerHTML=`<audio controls src="${url}"></audio>`;else v.innerHTML=`<iframe src="${url}"></iframe>`}
async function saveText(){try{let text=document.getElementById('editor').value;status(await api('save-text',{path:selected},new Blob([text],{type:'text/plain;charset=utf-8'})));refresh()}catch(e){status('FAILED: '+e.message)}}async function downloadSelected(){if(!selected)return;try{let b=await api('download',{path:selected},null,'blob'),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=selected.split('/').pop();a.click();setTimeout(()=>URL.revokeObjectURL(a.href),2000)}catch(e){status('FAILED: '+e.message)}}async function hashSelected(){try{status(await api('hash',{path:selected}))}catch(e){status('FAILED: '+e.message)}}
...
@app.post("/api/admin")
async def admin_action(request: Request, action: str = Query(...), path: str | None = Query(default=None), name: str | None = Query(default=None), newName: str | None = Query(default=None), size: int | None = Query(default=None), uploadId: str | None = Query(default=None), offset: int = Query(default=0), instanceId: str | None = Query(default=None)):
    if not auth(request):
        return JSONResponse(status_code=401, content={"ok": False, "error": "invalid or missing SWRLZ_ADMIN_TOKEN"})
    try:
        if action == "runtime":
            return {"ok": True, "version": VERSION, "instanceId": INSTANCE, "root": str(ROOT), "live": str(LIVE), "uploadAuthConfigured": bool(TOKEN), **lalm_presence()}
        if action == "list":
            base = safe_path(path)
