...
from api.chat_state import CONTRACT as STATE_CONTRACT, _clean_state, _read_blob, _write_blob
...
def _state_from_value(value: dict[str, Any] | None) -> tuple[int, dict[str, Any]]:
    if not value:
        return 0, {"version": 1, "currentId": "", "threads": []}
    revision = int(value.get("revision") or 0)
    raw_state = value.get("state")
    if not isinstance(raw_state, dict):
        return revision, {"version": 1, "currentId": "", "threads": []}
    return revision, _clean_state(raw_state)
...
def _commit_message(
    ...
    *,
    ...
    for attempt in range(MAX_COMMIT_ATTEMPTS):
        ...
        canonical["threads"] = sorted(
            threads,
            key=lambda item: (bool(item.get("pinned")), int(item.get("updatedAt") or 0)),
            reverse=True,
        )[:80]
        ...
        verified_revision, verified_state = _state_from_value(verified)
...
def begin_turn(request: Request, payload: dict[str, Any]) -> CanonicalTurn:
    ...
    revision = _commit_message(
        ...
        title_hint=_title_hint(prompt),
        extra_meta={"commitPhase": "PRE_GENERATION"},
    )
    return CanonicalTurn(
        user_id=user_id,
        account_scope=_scope(user_id),
        thread_id=thread_id,
        request_id=request_id,
...
def finish_turn(turn: CanonicalTurn, *, text: str, terminal_type: str, reason: str = "") -> int:
    ...
    return _commit_message(
        ...
        extra_meta={"commitPhase": "TERMINAL", "terminalType": terminal, "terminalReason": _bounded(reason, 2000)},
    )
