...
def _ensure_portal() -> None:
    ...
    _atomic_write(WEB_ROOT / "index.html", b"<!doctype html><meta charset='utf-8'><title>§wyrlz Runtime</title><p>Runtime source: runtime branch.</p>")


def _pages() -> list[dict[str, Any]]:
    WEB_ROOT.mkdir(parents=True, exist_ok=True)
    return [{"relative": p.relative_to(WEB_ROOT).as_posix(), "url": "/live/" + p.relative_to(WEB_ROOT).as_posix(), "size": p.stat().st_size} for p in sorted(WEB_ROOT.rglob("*.html")) if p.name != "index.html"]


def _control_html() -> str:
    return "<!doctype html><html><body style='font-family:system-ui;background:#050914;color:#eef7ff;padding:20px'><h1>Hot Runtime Control</h1><p>Source: <b>runtime</b>. Changes are fetched directly from the durable runtime branch.</p><input id='t' type='password' placeholder='SWRLZ_ADMIN_TOKEN'><button onclick='call("/api/hot/sync","POST")'>SYNC RUNTIME</button><button onclick='call("/api/hot/status")'>STATUS</button><pre id='o'>Ready.</pre><script>const t=document.querySelector('#t'),o=document.querySelector('#o');t.value=sessionStorage.getItem('swrlzAdminToken')||'';async function call(u,m='GET'){sessionStorage.setItem('swrlzAdminToken',t.value.trim());const r=await fetch(u,{method:m,headers:{'x-swrlz-admin-token':t.value.trim()},cache:'no-store'});const x=await r.text();try{o.textContent=JSON.stringify(JSON.parse(x),null,2)}catch{o.textContent=x}}</script></body></html>"
