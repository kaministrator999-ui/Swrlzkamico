...
def _local_stream(payload):
    ...
    try:
        ...
        profile_id=str(payload.get("profileId") or "")[:96]
        explicit_online=payload.get("onlineResearchRequested") if isinstance(payload.get("onlineResearchRequested"),bool) else None
        print("SWRLZ_BRAIN_R39_CALL_BOUNDARY "+json.dumps({"contract":"swrlz-brain-r39-call-boundary-camera-v1","stage":"generate-events-call","requestId":str(request_id)[:160],"engineRevision":str(getattr(engine,"HOT_REVISION",""))[:160],"profileId":profile_id,"profileOnline":bool(online_research_requested(profile_id)),"researchRequestedAtAdapter":bool(research_requested),"hasResearchPlan":isinstance(payload.get("researchPlan"),dict),"hasOnlineEvidence":isinstance(payload.get("onlineEvidence"),dict),"payloadOnlineResearchRequested":explicit_online},ensure_ascii=False,separators=(",",":")),flush=True)
