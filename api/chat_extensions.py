...
def _normalize_with_generation(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(raw, dict):
        ...
        try:
            ...
            if "maxTokens" in raw:
                generation["maxTokens"] = min(8192, max(1, int(raw["maxTokens"])))
...
chat.APP_VERSION = "1.3.4"
chat.app.version = "1.3.4"
...
def runtime_chat_state():
    now = time.time()
    engine, source = _engine()
    return {"available": True,"activeStreams": len(ACTIVE),"requests": [{**v, "ageSeconds": round(now - v["startedAt"], 2)} for v in ACTIVE.values()],"server": _server_state(),"bridgeVersion": chat.APP_VERSION,"localEngine":{"engineId": str(engine.ENGINE_ID),"modelSha256": str(engine.MODEL_SHA256),"source": source,"hotRevision": str(getattr(engine, "HOT_REVISION", "")),**LOCAL_READINESS}}
    ...
    try:
        ...
        try:
            for raw in _heartbeat_events(source_events):
                ...
                event = _event_payload(seq,request_id,raw); yield chat._encode_event(event); seq += 1
...
async def swrlz_chat_extensions(request: Request, call_next):
    ...
    if request.method == "GET" and path.endswith("/api/chat") and not request.query_params:
        page = hot_chat_path("chat.html", BUNDLED_CHAT_PAGE); html = page.read_text("utf-8")
        html = html.replace("</head>",'<link rel="stylesheet" href="/api/chat/assets/enhancements.css"></head>'); html = html.replace("</body>",'<script src="/api/chat/assets/enhancements.js"></script></body>')
