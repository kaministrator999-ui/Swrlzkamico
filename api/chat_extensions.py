...
_original_normalize=chat._normalize_chat_request; _original_status_payload=chat._status_payload; _original_verify_r39=chat._verify_r39; _original_forward_cancel=chat._forward_cancel

def _normalize_with_generation(payload: dict[str, Any]) -> dict[str, Any]:
    normalized=_original_normalize(payload); raw=payload.get("generation")
...
def _verify_r39():
    base=_original_verify_r39(); engine=inspect_engine(); LOCAL_READINESS.clear(); LOCAL_READINESS.update({"checked":True,**engine}); base["engine"]=engine; return base
chat._verify_r39=_verify_r39

def _event_payload(seq,request_id,raw):
    event_type=str(raw.get("type","STATUS")); terminal=event_type in chat.TERMINAL_TYPES; event=chat._bridge_event(seq,event_type,request_id,phase=str(raw.get("phase") or "STATE_VALIDATION"),reason=str(raw.get("reason") or ""),categories=[str(x) for x in raw.get("categories",[])][:12],terminal=terminal); identity=raw.get("identity")
...
def _stream_response(payload):
    ...
    iterator=chat._proxy_stream(payload) if upstream_ready else _local_stream(payload); headers=chat._no_store_headers(); headers.update({"X-SWRLZ-Stream-Contract":chat.STREAM_CONTRACT,"X-SWRLZ-Request-Id":payload["requestId"],"X-Accel-Buffering":"no"}); return StreamingResponse(iterator,media_type="application/x-ndjson",headers=headers)
...
async def swrlz_chat_extensions(request:Request,call_next):
    ...
    if request.method=="GET" and path.endswith("/api/chat") and not request.query_params:
        html=CHAT_PAGE.read_text("utf-8"); html=html.replace("</head>",'<link rel="stylesheet" href="/api/chat/assets/enhancements.css"></head>'); html=html.replace("</body>",'<script src="/api/chat/assets/enhancements.js"></script></body>'); return HTMLResponse(html,headers={"Cache-Control":"no-store","Content-Security-Policy":"default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'"})
...
async def enhancements_js():return FileResponse(ENH_JS,media_type="application/javascript",headers={"Cache-Control":"no-store"})
@chat.app.get("/assets/enhancements.css",include_in_schema=False)
async def enhancements_css():return FileResponse(ENH_CSS,media_type="text/css",headers={"Cache-Control":"no-store"})
