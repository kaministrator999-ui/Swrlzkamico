...
_TRANSPORT_WORKERS = max(1, min(8, int(os.environ.get("SWRLZ_R39_TRANSPORT_WORKERS", "6"))))
...
def _stage_verified_chunk(root, chunk, staging):
    ...
    return index, part, actual_size, origin


def _copy_verified_chunks(root, data, target):
    chunks = sorted(data.get("chunks") or [], key=lambda c: int(c["index"]))
    if not chunks:
    ...
    staging = LIVE / ".r39-transport-parts"
    if staging.exists():
        shutil.rmtree(staging, ignore_errors=True)
    staging.mkdir(parents=True, exist_ok=True)
    results = {}
    origins = set()
    workers = min(_TRANSPORT_WORKERS, len(chunks))
    try:
        with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="r39-chunk") as pool:
            futures = [pool.submit(_stage_verified_chunk, root, chunk, staging) for chunk in chunks]
            for future in as_completed(futures):
                index, part, actual_size, origin = future.result()
                results[index] = (part, actual_size)
                origins.add(origin)
        ...
        with target.open("wb") as out:
            for chunk in chunks:
                index = int(chunk["index"])
                part, actual_size = results[index]
        ...
        if total != expected_whole_size or digest != expected_whole_sha:
        ...
        try:
            ...
            if source_name == PACKED.name:
                ...
                return {
                    ...
                    "chunks": meta["chunks"],
                    "wrapper": "none",
                    "chunkOrigins": meta["origins"],
                    "transportWorkers": meta["workers"],
                    "packedVerified": True,
                }
...
def ensure_r39():
    ...
    if not PACKED.exists() and url:
        tmp = PACKED.with_suffix(PACKED.suffix + ".part")
        with urllib.request.urlopen(url, timeout=90) as r, tmp.open("wb") as out:
            while True:
                b = r.read(1024 * 1024)
                if not b:
                    break
    ...
    if not PACKED.exists():
        return {
            "ok": False,
            "modelReady": False,
            "code": "R39_NOT_PRESENT",
            "detail": "Upload R39 with Forge chunked Git transport (direct gzip or ZIP wrapper), upload the gzip through /api/admin, or set SWRLZ_R39_URL.",
        }
    ...
    packed_verified = bool(transport and transport.get("packedVerified"))
    psha = PACKED_SHA if packed_verified else sha(PACKED)
    if psha != PACKED_SHA:
    ...
    with gzip.open(PACKED, "rb") as src, tmp.open("wb") as dst:
        while True:
            ...
            raw_size += len(b)
    os.replace(tmp, RAW)
    rsha = raw_hash.hexdigest()
    ok = raw_size == RAW_SIZE and rsha == RAW_SHA
    if not ok:
        return {"ok": False, "modelReady": False, "code": "R39_RAW_VERIFY_FAILED", "actualSize": raw_size, "actualSha256": rsha}
    ...
    if transport:
        ...
        result["transportWorkers"] = transport.get("transportWorkers")
        result["packedHashReused"] = packed_verified
        if transport.get("archive"):
            result["transportArchive"] = transport["archive"]
        if transport.get("nested"):
            result["transportNestedR39"] = transport["nested"]
    return result
