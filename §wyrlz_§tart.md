...
When the decision is to resume/recontinue, the Roadmap must preserve the complete continuation lifecycle inside the existing governed event:

1. the original **UPDATE STARTED** marker remains the root of the event;
2. **before every resumed/recontinued work session performs any new implementation mutation**, append **UPDATE CONTINUATION STARTED** with the newly observed source/version/SHA/runtime baseline, what was already completed, what remains, changed assumptions/ownership if any, deployment boundary, and verification plan;
3. when that continuation session stops, append **UPDATE CONTINUATION ENDED** recording exactly where it stopped, what changed during that continuation, current versions/truth state, verification/deployment state, and what remains. If the overall update reaches a terminal state in that same continuation, the terminal **UPDATE FINISHED**, **ABORTED**, or **SUPERSEDED** marker serves as the continuation end and a redundant CONTINUATION ENDED marker is not required;
4. every later resume repeats the continuation-start → continuation-end/terminal-marker pair.

A continuation checkpoint is lineage inside the original governed event; it does not silently create a second overlapping event or reserve stale version numbers. **No resumed/recontinued work session may disappear between markers.** Detailed continuation-journal fields remain owned by the Version Evolution contract; this router enforces the lifecycle before and after resumed mutation.
