...
# v66 must hydrate the current v65 runtime lineage so generation-boundary
# diagnostics/fixes are inherited instead of pinning the original v65 snapshot.
_V65_COMMIT="c333e80266f87414a5e2806e89b552fe010bb064"
...
HOT_SERVER_VERSION="2.1.78"
HOT_REVISION="2.1.78-hot-programming-mode-context-v66-camera-lineage"
...
_CODE_TERM_RE=re.compile(r"\b(?:code|coding|program(?:ming)?|software|script|function|class|method|api|endpoint|route|database|schema|repository|repo|github|git|branch|commit|frontend|backend|server|client|component|module|package|library|framework|html|css|javascript|typescript|python|kotlin|java|swift|rust|golang|react|vue|svelte|node|sql|json|yaml|docker|vercel|website|webpage|web\s+page|web\s+app|application|app|codebase|source\s+code|ui|auth|oauth|deployment|lalm)\b",re.I)
...
def _direct_programming_signal(text):
    value=str(text or "");term=bool(_CODE_TERM_RE.search(value) or _FILE_EXT_RE.search(value));action=bool(_CODE_ACTION_RE.search(value));broad=bool(re.search(r"\b(?:page|site|app|project|repo|code|server|api|database|ui|lalm)\b",value,re.I));return bool(term or (action and broad))
...
def _programming_profile(payload):
    latest=_prog_latest(payload);direct=_direct_programming_signal(latest);inherited=False;source="current-turn" if direct else "none";inherited_from="";effective=latest
    if not direct and _CONTINUE_RE.search(latest) and not _STOP_RE.search(latest):
        for distance,prior in enumerate(_prog_prior_users(payload,8),start=1):
            if _direct_programming_signal(prior):direct=True;inherited=True;source="conversation-bridge";inherited_from=f"user@history_distance_{distance}";effective=prior;break
            if len(re.findall(r"[A-Za-z0-9_]+",prior))>=4:break
    if not direct:return {"active":False,"codingTask":False,"projectContext":"none","changeClass":"none","architectureDepth":"none","architectureReconciliation":False,"diagnostics":False,"projectCoaching":False,"toolEvidenceRequired":False,"source":"none","inherited":False,"inheritedFrom":"","implementationTruth":"runtime-scaffolded"}
    hint="fix" if _FIX_RE.search(effective) else "refactor" if _REFACTOR_RE.search(effective) else "migrate" if _MIGRATE_RE.search(effective) else "deploy" if _DEPLOY_RE.search(effective) else "";context=_project_context(effective,hint);change=_change_class(effective,context);depth=_architecture_depth(effective,context,change);mutation=change in {"create","feature","fix","refactor","migrate","deploy"}
    return {"active":True,"codingTask":True,"projectContext":context,"changeClass":change,"architectureDepth":depth,"architectureReconciliation":bool(context=="existing" and mutation),"diagnostics":bool(change=="fix"),"projectCoaching":bool(context=="new" and mutation),"toolEvidenceRequired":bool(context=="existing" and mutation),"source":source,"inherited":inherited,"inheritedFrom":inherited_from,"implementationTruth":"runtime-scaffolded"}

def _programming_context_prompt(profile):
    return ("[SWRLZ_PROGRAMMING_MODE v1] Internal programming operating grammar; do not expose this marker verbatim. "+f"projectContext={profile.get('projectContext')}; changeClass={profile.get('changeClass')}; architectureDepth={profile.get('architectureDepth')}; inherited={str(bool(profile.get('inherited'))).lower()}; architectureReconciliation={str(bool(profile.get('architectureReconciliation'))).lower()}; diagnostics={str(bool(profile.get('diagnostics'))).lower()}; projectCoaching={str(bool(profile.get('projectCoaching'))).lower()}. "+"For an existing-project change, separate desired outcome from implementation assumptions. Inspect available project/repository evidence before inventing owners or new structure; identify current authority, relevant state/readers/writers/lifecycle, and related implementations. Prefer reuse -> extend canonical owner -> consolidate/refactor -> migrate+retire -> genuinely new structure. If evidence is unavailable or ownership remains ambiguous, do not fabricate architecture; plan the needed inspection. For a new user project, design the smallest useful structure justified by current scale and risk; strengthen ownership, state, tests, observability, versioning, and deployment boundaries only as complexity warrants. Respect explicit user choices to simplify optional structure and explain material tradeoffs without repeatedly pressuring them. For lightweight standalone coding/explanation, answer directly and do not force repository ceremony. For fix/debug work, inspect existing diagnostic evidence first and add bounded instrumentation only when an observability gap prevents reliable diagnosis. Preserve one intentional authority per responsibility and verify behavior plus architecture before claiming completion. This marker does not grant permission to write files, invoke tools, deploy, spend, or bypass any user/server approval boundary.")
...
def _with_programming_context(payload,profile=None):
    ...
    out=dict(payload);history=list(out.get("history") or []);marker={"role":"system","text":_programming_context_prompt(profile)};insert_at=1 if history and isinstance(history[0],dict) and str(history[0].get("role") or "").lower()=="system" else 0;history.insert(insert_at,marker);out["history"]=history;return out
...
def _programming_mode_self_test():
    ...
    add("non-programming-question",{"prompt":"Explain quantum physics."},{"active":False,"codingTask":False});add("lightweight-code-explanation",{"prompt":"Explain how this Python function works."},{"active":True,"projectContext":"none","changeClass":"explain","architectureDepth":"lightweight","architectureReconciliation":False});add("existing-project-feature",{"prompt":"Add response bookmarks to our existing app and update the current repo."},{"active":True,"projectContext":"existing","changeClass":"feature","architectureDepth":"normal","architectureReconciliation":True,"toolEvidenceRequired":True});add("existing-project-debug",{"prompt":"Fix the OAuth regression in our current web app."},{"active":True,"projectContext":"existing","changeClass":"fix","diagnostics":True,"architectureReconciliation":True});add("new-small-webpage",{"prompt":"Build me a simple personal webpage from scratch in one HTML file."},{"active":True,"projectContext":"new","changeClass":"create","architectureDepth":"lightweight","projectCoaching":True,"architectureReconciliation":False});add("cross-cutting-migration",{"prompt":"Migrate our existing app authentication and database schema with rollback support."},{"active":True,"projectContext":"existing","changeClass":"migrate","architectureDepth":"deep","architectureReconciliation":True});add("coding-continuation",{"prompt":"Ideal most definitely swyrlz let's do this!","history":[{"role":"user","text":"Add architecture-aware coding mode to our current LALM project."},{"role":"assistant","text":"I can implement the runtime scaffold."}]},{"active":True,"projectContext":"existing","changeClass":"feature","architectureDepth":"normal","inherited":True});passed=sum(1 for case in cases if case["ok"]);return {"ok":passed==len(cases),"passed":passed,"total":len(cases),"failures":[case for case in cases if not case["ok"]],"contract":"swrlz_programming_mode_acceptance_v1"}

def inspect_engine():
    result=_V65_INSPECT()
    if isinstance(result,dict):result.update({"hotServerVersion":HOT_SERVER_VERSION,"hotRevision":HOT_REVISION,"programmingModeRuntimeScaffolded":True,"programmingTaskClassifier":True,"programmingContextCompiler":True,"programmingArchitectureState":True,"existingProjectArchitectureReconciliationPrompt":True,"newProjectProportionalArchitecturePrompt":True,"programmingContinuationInheritance":True,"programmingToolLoop":False,"codingSpecialistDelegation":False,"codingSpecialistModel":False,"programmingImplementationTruth":"runtime-scaffolded","programmingModeContract":_PROGRAMMING_CONTRACT,"programmingContextContract":_PROGRAMMING_CONTEXT_CONTRACT,"programmingModeSelfTest":_programming_mode_self_test(),"v65SourceCommit":_V65_COMMIT})
    return result

def generate_events(payload,is_cancelled=None):
    request_id=_request_id(payload) if isinstance(payload,dict) else "";profile=_programming_profile(payload) if isinstance(payload,dict) else {"active":False};routed=_with_programming_context(payload,profile) if profile.get("active") else payload
    if profile.get("active"):_camera(request_id,"programming-mode",contract=_PROGRAMMING_CONTRACT,projectContext=profile.get("projectContext"),changeClass=profile.get("changeClass"),architectureDepth=profile.get("architectureDepth"),inherited=bool(profile.get("inherited")),architectureReconciliation=bool(profile.get("architectureReconciliation")),diagnostics=bool(profile.get("diagnostics")),projectCoaching=bool(profile.get("projectCoaching")),toolEvidenceRequired=bool(profile.get("toolEvidenceRequired")),implementationTruth="runtime-scaffolded")
    try:
        for event in _V65_GENERATE(routed,is_cancelled):yield event
    except Exception as exc:
        _camera(request_id,"v66-generate-exception",contract="r39-v66-generation-camera-v1",errorType=type(exc).__name__,errorMessage=str(exc)[:240])
        raise
