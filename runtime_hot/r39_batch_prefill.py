"""Vectorized R39 prompt prefill with persistent dense-weight fast path.

The direct-quantized matmat kernel remains the safe fallback. Eligible weights are decoded
once per warm worker and retained as float32 matrices. Prompt attention is evaluated as
one masked block operation instead of a Python loop over every query token.
"""
...
def cache_stats() -> dict[str, int]:
    with _DENSE_LOCK:
        return {"items":len(_DENSE_CACHE),"bytes":int(_DENSE_CACHE_BYTES),"budgetBytes":int(_DENSE_CACHE_BUDGET),"hits":int(_DENSE_CACHE_HITS),"misses":int(_DENSE_CACHE_MISSES)}
...
def _dense_matrix(model, name: str, batch: int) -> np.ndarray | None:
    ...
    d=model.desc[name]; shape=d["shape"]
    cols=int(shape[0]); rows=int(shape[1]) if len(shape)>1 else 1
    decoded_bytes=rows*cols*4
    if batch < _DENSE_MATERIALIZE_MIN_BATCH or decoded_bytes > _DENSE_ITEM_MAX: return None
    with _DENSE_LOCK:
        if _DENSE_CACHE_BYTES + decoded_bytes > _DENSE_CACHE_BUDGET: return None
    value=np.ascontiguousarray(model.matrix(name),dtype=np.float32)
    if value.ndim != 2 or value.shape != (rows,cols): return None
    with _DENSE_LOCK:
        existing=_DENSE_CACHE.get(key)
        if existing is not None: return existing
        if _DENSE_CACHE_BYTES + int(value.nbytes) > _DENSE_CACHE_BUDGET: return None
        _DENSE_CACHE[key]=value; _DENSE_CACHE_BYTES += int(value.nbytes)
        return value
...
def _matmat(bridge,model,name:str,x:np.ndarray)->np.ndarray:
    ...
    if dense is not None: return np.asarray(dense @ value,dtype=np.float32)
    out=bridge.matmat(model,name,value)
    if out is None: raise RuntimeError(f"R39_BATCH_MATMAT_UNAVAILABLE:{name}")
...
def _head_rms_cols(x,heads,weight):
    tokens=int(x.shape[1]); a=np.asarray(x,dtype=np.float32).reshape(int(heads),64,tokens)
...
def _rope_cols(x,heads,start_pos,theta=1_000_000.0):
    tokens=int(x.shape[1]); y=np.asarray(x,dtype=np.float32).T.reshape(tokens,int(heads),64).copy(); half=32
    ...
    c=np.cos(angles).astype(np.float32)[:,None,:]; s=np.sin(angles).astype(np.float32)[:,None,:]
...
def forward_token_block(base,bridge,model,state,tokens:list[int])->np.ndarray:
    if not tokens: raise ValueError("tokens must be non-empty")
    start_pos=int(state.pos); count=len(tokens)
    ...
    for i,kvh in enumerate(base.LAYER_KV):
        ...
        if kvh==0:
            projected=_matmat(bridge,model,f"blk.{i}.shortconv.in_proj.weight",n); b,c,z=np.split(projected,3,axis=0); bx=(b*z).astype(np.float32)
            cw=np.asarray(model.matrix(f"blk.{i}.shortconv.conv.weight"),dtype=np.float32); hist=np.asarray(state.conv[i],dtype=np.float32); seq=np.concatenate((hist,bx.T),axis=0)
            cv=(seq[0:count].T*cw[:,0:1]+seq[1:count+1].T*cw[:,1:2]+seq[2:count+2].T*cw[:,2:3]).astype(np.float32)
            state.conv[i][0]=seq[-2].copy(); state.conv[i][1]=seq[-1].copy(); op=_matmat(bridge,model,f"blk.{i}.shortconv.out_proj.weight",(c*cv).astype(np.float32))
        else:
            q=_matmat(bridge,model,f"blk.{i}.attn_q.weight",n); k=_matmat(bridge,model,f"blk.{i}.attn_k.weight",n); v=_matmat(bridge,model,f"blk.{i}.attn_v.weight",n)
            q=_head_rms_cols(q,16,model.vector(f"blk.{i}.attn_q_norm.weight")); k=_head_rms_cols(k,8,model.vector(f"blk.{i}.attn_k_norm.weight")); q=_rope_cols(q,16,start_pos); k=_rope_cols(k,8,start_pos)
            old_entries=list(state.kv[i]); att=_causal_gqa(q,k,v,old_entries); state.kv[i].extend((k[:,t].copy(),v[:,t].copy()) for t in range(count)); op=_matmat(bridge,model,f"blk.{i}.attn_output.weight",att)
        residual=(x+op).astype(np.float32); fn=_rms_cols(residual,model.vector(f"blk.{i}.ffn_norm.weight")); gate=_matmat(bridge,model,f"blk.{i}.ffn_gate.weight",fn); up=_matmat(bridge,model,f"blk.{i}.ffn_up.weight",fn)
        ff=_matmat(bridge,model,f"blk.{i}.ffn_down.weight",(base._silu(gate)*up).astype(np.float32)); x=(residual+ff).astype(np.float32)
    hidden=_rms_cols(x,model.vector("token_embd_norm.weight")); state.pos=start_pos+count
