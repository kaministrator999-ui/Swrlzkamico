...
_PERSISTENT_ROUTINE = _PERSISTENT_ROUTINE + (
    " Preserve received user text as the receipt. When a token is literally valid but semantically anomalous, "
    "infer the smallest plausible repair from surrounding context and assign confidence before using it. "
    "High-confidence repairs may be surfaced as 'I think you meant X'; uncertain repairs remain ambiguous. "
    "This is general contextual repair, not an STT-only spellcheck. If provenance is supplied, treat modality, "
    "STT confidence, and candidate alternatives as evidence that updates repair probability, never as authority. "
    "When a response contains copyable code, commands, lyrics, structured text, or other composition whose exact "
    "whitespace matters, use a fenced code block so the UI can expose a copy control. "
)


def _adaptive_generation(generation,profile):
    g,old=_V35_BASE_ADAPTIVE(generation,profile)
    requested=max(32,int((generation or {}).get("maxTokens",512)))
...
def _controlled_payload(payload):
    ...
    profile=dict(clone.get("_adaptiveProfile") or {})
    generation,plan=_adaptive_generation(clone.get("generation") if isinstance(clone.get("generation"),dict) else {},profile)
    clone["generation"]=generation
    behavior=dict(clone.get("_conversationBehavior") or {})
    behavior.update({"softTargetTokens":plan["softTargetTokens"],"emergencyCeilingTokens":plan["emergencyCeilingTokens"],"wrapUpPolicy":"finish-open-structure-before-natural-eos","planningHorizon":"median-not-quota","copyableCompositionPolicy":"fenced-code-for-exact-copy-content","contextualRepairPolicy":"receipt-preserved-smallest-plausible-repair-with-confidence"})
    clone["_conversationBehavior"]=behavior
    clone["_adaptiveProfile"]={**profile,**plan}
    return clone,contract,info


def generate_events(payload,is_cancelled=None):
    profile=_message_profile(str(payload.get("prompt") or ""))
    _,plan=_adaptive_generation(payload.get("generation") if isinstance(payload.get("generation"),dict) else {},profile)
    ...
    for event in _V35_BASE_GENERATE(payload,is_cancelled):
        if isinstance(event,dict) and event.get("type")=="DELTA":
            ...
            text+=chunk
            approx_words=len(re.findall(r"\S+",text))
            state=_structure_state(text)
            if not horizon_reported and approx_words>=plan["softTargetTokens"]:
                horizon_reported=True
                yield {"type":"STATUS","phase":"RESPONSE_HORIZON","reason":f"soft planning horizon reached · approxWords={approx_words} · scale={profile['responseScale']} · openStructure={any(state[k] for k in ('openFence','openParen','openBracket','openBrace'))} · naturalEosPreferred=true"}
            if not wrap_reported and approx_words>=max(1,plan["softTargetTokens"]-16) and any(state[k] for k in ('openFence','openParen','openBracket','openBrace')):
                wrap_reported=True
                yield {"type":"STATUS","phase":"WRAP_UP_INSTRUMENTATION","reason":"Near the soft horizon with an open output structure; continue until the current structure can close naturally rather than cutting the response."}
        yield event
    if not horizon_reported:
        yield {"type":"STATUS","phase":"RESPONSE_HORIZON","reason":f"response ended naturally before soft horizon · scale={profile['responseScale']} · naturalEosPreferred=true"}


_impl.HOT_SERVER_VERSION="2.1.44"
_impl.HOT_REVISION="2.1.44-hot-boundary-v35-soft-median-response-horizon-v5.1"
