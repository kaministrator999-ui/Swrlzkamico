"""R39 v40: import-safe isolated telemetry wrapper over v37.

v39 correctly isolated v37's globals to prevent _BASE_GENERATE recursion, but the
isolated ModuleType was not registered in sys.modules before executing the inherited
source chain. Python runtime features used by inherited code may resolve class/module
metadata through sys.modules during execution, so an unregistered synthetic module can
fail at import before generate_events is reachable.

v40 registers the isolated namespace before exec while preserving v39's observation-only
server telemetry and the v37 inference behavior unchanged.
"""
...
_req = urllib.request.Request(_BASE_URL, headers={"User-Agent": "swrlz-r39-v40"})
...
_MODULE_NAME = "swrlz_r39_v37_isolated_v40"
_v37 = types.ModuleType(_MODULE_NAME)
_v37.__file__ = _BASE_URL
_v37.__package__ = ""
sys.modules[_MODULE_NAME] = _v37
try:
    exec(compile(_source.decode("utf-8"), _BASE_URL + "#v40-base", "exec"), _v37.__dict__, _v37.__dict__)
except Exception:
...
HOT_SERVER_VERSION = "2.1.51"
HOT_REVISION = "2.1.51-hot-import-safe-server-telemetry-v40"
...
def inspect_engine():
    ...
    if isinstance(result, dict):
        result.update({
            ...
            "telemetryWrapperNamespace": "registered-isolated-v37",
            "isolatedModuleRegistered": True,
        })
    return result
