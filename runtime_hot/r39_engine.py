"""Hot R39 entrypoint v75 with continuation provenance and runnable edit semantic gates."""
...
def _entry(stage,**fields):
    record={"contract":"r39-hot-entry-camera-v1","stage":stage,"target":"v75","atUnixMs":int(time.time()*1000)}
...
try:
    _entry("fetch-start",sourceCommit=_V74_COMMIT,overlayCommit=_V75_COMMIT)
    ...
    _entry("hydrate-ok",hotServerVersion=str(globals().get("HOT_SERVER_VERSION") or ""),
           ...
           artifactContinuation=True,continuationProvenance=True,runnableEditSemanticGate=True,selfTest=True)
