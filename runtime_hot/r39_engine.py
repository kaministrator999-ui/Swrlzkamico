"""Hot R39 v23 grounded identity slot + canonical append cursor.

Keeps the v22 hard assistant-name fact, but no longer forces the name as the opening
response. Identity intents receive a compact runtime grounding slot and normal R39
language generation remains authoritative. A verifier prevents role labels from becoming
the assistant identity and repairs a missing canonical name only at completion.
Prefill telemetry is further reduced without changing recurrent math.
"""
...
_req=urllib.request.Request(_IMPL_URL,headers={"User-Agent":"swrlz-hot-r39-v23"})
...
_new_eos='''            if model.tokenizer.eos is not None and next_token == int(model.tokenizer.eos):
                ...
                break
'''
if _old_eos not in _source_text:raise RuntimeError("R39_V23_EOS_PATCH_TARGET_MISSING")
...
'''
_new_lookup='''        with _PREFIX_LOCK:
            entries=[{"tokens":list(x.get("tokens") or []),"usedAt":float(x.get("usedAt",0.0))} for x in _PREFIX_CACHE]
        entries.sort(key=lambda x:x["usedAt"],reverse=True)
        best=None; rows=[]; now=time.time()
        ...
        if best is not None:
            ...
            if lcp < min(len(cached),len(tokens)):
                yield {"type":"STATUS","phase":"PREFIX_MISMATCH","reason":f"best resident entry #{best['index']} diverges at token {lcp+1}; cached={int(cached[lcp])}:{_token_text(model,int(cached[lcp]))!r}, current={int(tokens[lcp])}:{_token_text(model,int(tokens[lcp]))!r}."}
            elif len(cached)<=len(tokens):
                yield {"type":"STATUS","phase":"PREFIX_MATCH","reason":f"best resident entry #{best['index']} is an exact {len(cached)}-token prefix of {len(tokens)}; only appended suffix requires prefill."}
            else:
                yield {"type":"STATUS","phase":"PREFIX_TRUNCATION","reason":f"current request ends at {lcp} token(s) while resident entry #{best['index']} continues to {len(cached)}."}
        prefix_len,cached_state,cached_logits=_prefix_get(tokens)
        state=cached_state if cached_state is not None else base.RecurrentState()
'''
if _old_lookup not in _source_text:raise RuntimeError("R39_V23_PREFIX_PATCH_TARGET_MISSING")
...
'''
if _old_yield not in _source_text:raise RuntimeError("R39_V23_PREFILL_PATCH_TARGET_MISSING")
_source_text=_source_text.replace(_old_yield,_new_yield,1)

_impl=types.ModuleType("swrlz_hot_r39_engine_impl_v23");_impl.__file__=_IMPL_URL
exec(compile(_source_text,_IMPL_URL,"exec"),_impl.__dict__)
_impl.HOT_SERVER_VERSION="2.1.32"
_impl.HOT_REVISION="2.1.32-hot-boundary-v23-grounded-identity-slot-v2.1"
...
_PERSISTENT_ROUTINE="RC2.1 runtime facts are authoritative semantic slots; generate natural language around them. Human=user; role labels are never names. User alone controls optional nickname; alias never replaces canonical identity. Ground answers in conversation facts; diagnostic=causes>evidence>root-cause; verification=facts!=inference; preserve invariants; reasoning-depth!=output-length; mutation-authority-explicit; never-infer-deploy-authority; answer-directly"

def _controlled_payload(payload):
    clone=dict(payload);history=[dict(t) for t in list(clone.get("history") or []) if isinstance(t,dict)]
    identity=_needs_canonical_name(str(clone.get("prompt") or ""))
    routine=_PERSISTENT_ROUTINE
    if identity:routine += f"; FACT assistant.name={_CANONICAL_NAME}; answer the user's identity question naturally and include this exact name as the identity fact"
    clone["history"]=[{"role":"SYSTEM","text":routine}]+history
    clone["_identitySlotRequired"]=identity
...
def _selection_evidence(reason,context):
    ...
    return f"selected={chosen!r} · lexicalOccurrences={text.count(needle)} · lastOccurrenceOffset={text.rfind(needle)} · roleLikeToken={needle in {'user','assistant','system','ai'}} · evidenceOnly=true; occurrence is not proof of causal attribution"
...
def generate_events(payload,is_cancelled=None):
    controlled,c=_controlled_payload(payload);axes=",".join(c["axes"]) or "defaults";modes="+".join(c["modes"]) or "GENERAL";objs="+".join(c["objectives"]) or "SATISFY_INTENT";context=_visible_context(payload);identity=bool(controlled.get("_identitySlotRequired"));pieces=[]
    yield {"type":"STATUS","phase":"REASONING_CONTRACT","reason":f"v2.1 grounded-slots · axes={axes} · mode={modes} · objective={objs} · mutation={c['mutation']} · verify={c['verification']} · presentation={c['length']}"}
    if identity:yield {"type":"STATUS","phase":"HARD_VARIABLE_RESOLVED","reason":f"assistant.name={_CANONICAL_NAME!r} resolved as required semantic slot; normal language generation remains enabled."}
    completed=None
    ...
    if identity:
        rendered="".join(pieces);low=rendered.strip().lower()
        bad=low in {"user","ai","assistant","system"}
        missing=_CANONICAL_NAME not in rendered
        if bad or missing:
            # Completion-level repair is fallback only. It preserves natural generation when valid,
            # while guaranteeing the hard fact is never replaced by a role label.
            ...
            yield {"type":"STATUS","phase":"GROUNDING_VERIFICATION","reason":"assistant.name invariant satisfied by natural generation; no repair required."}
    if completed is not None:yield completed

def inspect_engine():
    result=_impl.inspect_engine()
    if isinstance(result,dict):result.update({"hotServerVersion":HOT_SERVER_VERSION,"hotRevision":HOT_REVISION,"reasoningControl":True,"reasoningControlSpecVersion":"2.1","persistentReasoningRoutine":True,"assistantIdentity":_CANONICAL_NAME,"hardRuntimeVariables":True,"identitySlotGrounding":True,"identityForcedAsOpeningTokens":False,"naturalIdentityLanguage":True,"groundingVerification":True,"canonicalIdentityImmutable":True,"nicknameAuthority":"user-only","nicknameSelfAssignmentAllowed":False,"nicknameReplacesCanonicalIdentity":False,"roleLabelsAreNotIdentity":True,"selectionContextEvidence":True,"perTurnReasoningDirectiveInjection":False,"canonicalAppendCursor":True,"syntheticBosStrippedFromAppendFraming":True,"prefixDivergenceDiagnostics":True,"prefillTelemetryStride":32,"prefillSkipsIntermediateLogits":True,"sameWorkerAppendOptimized":True,"crossWorkerCursorPersistence":False,"speculativeDecodeOverPrefill":False})
    return result
