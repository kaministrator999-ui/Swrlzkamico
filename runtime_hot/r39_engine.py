...
v15 keeps pipelined conversation checkpoints and adaptive response budgets, adds
progressive prompt-prefill checkpoints for interruption recovery, strengthens coding
quality/consistency guidance and semantic checks, and reduces grouped-attention
Python overhead during token decoding.
"""
...
HOT_SERVER_VERSION = "2.1.24"
HOT_REVISION = "2.1.24-hot-boundary-v15-progressive-prefill-code-verify"
...
_STABLE_RESPONSE_DIRECTIVE = (
    "You are §wyrlz. Answer directly, truthfully, completely, and with a natural lightly personable tone when appropriate. "
    "Fulfill every explicit requested part before ending. Do not introduce or describe your model identity unless the user asks. "
    "For programming requests, prioritize runnable correctness before commentary: silently check formulas, operators, types, control flow, syntax, examples, and stated input/output behavior. "
    "Keep every code explanation consistent with the code actually shown; never claim user input when an example hardcodes a value. Finish requested code and explanations before ending."
)
    ...
    with _CONTEXT_LOCK:
        ...
        for checkpoint in checkpoints:
            ...
            if prefix_len > len(tokens) or tuple(tokens[:prefix_len]) != cached_tokens:
                continue
            if prefix_len == len(tokens) and not isinstance(checkpoint.get("logits"), np.ndarray):
                continue
...
def _forward_hot(model: base.R39Model, token: int, state: base.RecurrentState, *, need_logits: bool) -> np.ndarray | None:
    ...
    for i, kvh in enumerate(base.LAYER_KV):
        ...
        if kvh == 0:
            ...
            values_b = values.transpose(1, 0, 2)
            att = np.matmul(weights_b.transpose(0, 2, 1), values_b).astype(np.float32).reshape(16, 64)
...
def _publish_generated_checkpoint(model, thread_id, prompt, prompt_tokens, generated_text, generated_tokens, state, logits, *, kind):
    ...
    try:
        ...
        if len(open_tokens) != expected_len or tuple(open_tokens[:len(prompt_tokens)]) != tuple(prompt_tokens) or tuple(open_tokens[len(prompt_tokens):]) != tuple(generated_tokens):
            return False
...
def _plan_response_budget(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    kind = "normal"
    planned, wrap_at, hard = 192, 152, 384
    ...
    if str(generation.get("budgetMode") or "").lower() == "manual":
        try:
            manual = max(1, int(generation.get("maxTokens"))); hard = min(_ABSOLUTE_RESPONSE_HARD_CAP, manual); planned = min(planned, hard); wrap_at = min(wrap_at, max(1, planned - max(8, planned // 5))); kind = "manual"
        except Exception:
            pass
    planned = min(planned, _ABSOLUTE_RESPONSE_HARD_CAP); wrap_at = min(max(1, wrap_at), planned); hard = min(max(planned + 32, hard), _ABSOLUTE_RESPONSE_HARD_CAP)
...
def _looks_structurally_open(text: str) -> bool:
    ...
    if value.count("{") - value.count("}") > 0 or value.count("(") - value.count(")") > 0 or value.count("[") - value.count("]") > 0: return True
    tail = value.rstrip()
    if tail.endswith((":", ",", "\\", "->", "=>")): return True
    last_line = tail.splitlines()[-1].strip() if tail else ""
    return last_line.startswith(("- ", "* ")) or (last_line[:2].isdigit() and last_line.endswith("."))
...
def _response_contract(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    requirements = []
    if coding: requirements.extend(("code", "code-consistency"))
    if coding and explain: requirements.append("code-explanation")
    if coding and "function" in lower and "main" in lower and explain: requirements.append("function-main-explanation")
    if "fahrenheit" in lower and "celsius" in lower: requirements.append("fahrenheit-celsius-correctness")
...
def _fahrenheit_formula_looks_correct(code: str) -> bool:
    ...
    if "fahrenheit" not in compact and "f-32" not in compact: return False
...
def _response_contract_gaps(text: str, contract: dict[str, Any]) -> list[str]:
    value = str(text or ""); lower = value.lower(); code, prose = _split_code_and_prose(value); code_lower = code.lower(); prose_lower = prose.lower(); requirements = list(contract.get("requirements") or []); gaps = []
    if "code" in requirements:
        ...
        if not (fenced_complete or code_like): gaps.append("complete-code")
    if "code-explanation" in requirements and len(re.findall(r"[A-Za-z]{2,}", prose)) < 12: gaps.append("requested-explanation")
    if "function-main-explanation" in requirements and ("function" not in prose_lower or "main" not in prose_lower): gaps.append("function-and-main-explanation")
    if "code-consistency" in requirements and code:
        ...
        if user_input_claim and not actual_input: gaps.append("code-explanation-input-mismatch")
    if "fahrenheit-celsius-correctness" in requirements and code:
        if not _fahrenheit_formula_looks_correct(code): gaps.append("fahrenheit-celsius-formula")
        prose_compact = re.sub(r"\s+", "", prose_lower)
        if "f-32" in prose_compact and ("/5" in prose_compact or "}{5}" in prose_compact) and "/9" not in prose_compact and "}{9}" not in prose_compact: gaps.append("fahrenheit-celsius-explanation")
    return gaps


def _generate_hot_events(payload: dict[str, Any], is_cancelled=None):
    request_id = str(payload["requestId"]); thread_id = str(payload.get("threadId") or "").strip()[:128]; started = time.monotonic(); engine_entered_ms = int(time.time() * 1000)
    try: client_sent_ms = int(payload.get("clientSentAtMs") or 0)
    except Exception: client_sent_ms = 0
    approx_client_to_hot_ms = max(0, engine_entered_ms - client_sent_ms) if client_sent_ms > 0 else None
    try:
        already_warm = _MODEL is not None; native = native_bridge.available(); mode = "compiled direct-quantized kernels" if native else "Python/Numpy fallback (native extension not present on this worker)"; warm_note = "Reusing warm server R39 model." if already_warm else "Opening R39 once for this server worker; subsequent requests reuse the warm model."
        diag = f"Hot engine entered · server {HOT_SERVER_VERSION} · revision {HOT_REVISION} · {mode} · serverEpochMs {engine_entered_ms}"
        if approx_client_to_hot_ms is not None: diag += f" · approx client→hot {approx_client_to_hot_ms}ms"
        ...
        budget = _plan_response_budget(payload); contract = _response_contract(payload)
        ...
        if contract["requirements"]: yield {"type":"STATUS","phase":"RESPONSE_CONTRACT","reason":"Response completion contract armed · required: " + ", ".join(contract["requirements"]) + "."}
        prompt = base.render_chat_prompt(payload); tokens = model.tokenizer.encode(prompt)
        if not tokens: raise base.R39InferenceError("R39_PROMPT_TOKENIZATION_EMPTY","Prompt tokenization produced no tokens.")
        generation = payload.get("generation") if isinstance(payload.get("generation"), dict) else {}; temperature = min(2.0,max(0.0,float(generation.get("temperature",0.1)))); top_p = min(1.0,max(0.01,float(generation.get("topP",0.9))))
        state, reused, cached_logits, cache_reason = _resume_context(thread_id, tokens); pending = tokens[reused:]
        if reused: yield {"type":"STATUS","phase":"PREFILL","reason":f"Conversation prefill checkpoint hit: reused {reused} token(s); only {len(pending)} new token(s) require prefill."}
        elif cache_reason == "prefix-mismatch": yield {"type":"STATUS","phase":"PREFILL","reason":f"No cached checkpoint matches the edited/divergent prefix; safely rebuilding all {len(tokens)} token(s)."}
        else: yield {"type":"STATUS","phase":"PREFILL","reason":f"Prefilling {len(tokens)} token(s); progressive recovery checkpoints every {_PREFILL_CHECKPOINT_EVERY} token(s); backend={('native-qmatvec' if native else 'python-fallback')}."}
        logits = cached_logits if not pending else None; prefill_started = time.monotonic(); total_new = len(pending)
        for ordinal, token in enumerate(pending, start=1):
            ...
            token_started = time.monotonic(); need_logits = ordinal == total_new; step_logits = _forward_hot(model, token, state, need_logits=need_logits)
            if need_logits: logits = step_logits
            absolute_prefix = reused + ordinal
            if ordinal < total_new and ordinal % _PREFILL_CHECKPOINT_EVERY == 0: _store_context(thread_id, tokens[:absolute_prefix], state, None, kind=f"prefill-progress-{absolute_prefix}")
            token_s = time.monotonic()-token_started; elapsed = time.monotonic()-prefill_started; avg = elapsed/ordinal; eta = max(0.0, avg*(total_new-ordinal))
            if ordinal <= 2 or ordinal == total_new or ordinal % 32 == 0: yield {"type":"STATUS","phase":"PREFILL","reason":f"Prefill new {ordinal}/{total_new} · cached {reused} · {token_s:.3f}s token · {avg:.3f}s avg · ETA {eta:.1f}s · backend={('native' if native else 'python')}."}
        if logits is None: raise base.R39InferenceError("R39_CONTEXT_CACHE_LOGITS_MISSING","Conversation context cache did not provide terminal prefill logits.")
        cached = _store_context(thread_id,tokens,state,logits,kind="prompt"); state = _clone_state(state) if cached else state
        decoder = base.IncrementalDecoder(model.tokenizer); generated_tokens=[]; response_parts=[]; recent=[]; first_ms=None; prefill_time=time.monotonic()-prefill_started; cache_note=f"reused {reused}, prefetched {total_new}" if reused else f"prefetched {total_new}"
        if not cached and len(tokens)>_CONTEXT_MAX_TOKENS: cache_note += f"; cache skipped above {_CONTEXT_MAX_TOKENS}-token safety bound"
        ...
        ordinal=0; current_limit=int(budget["hard"]); wrap_notified=False; extended_for_structure=False; extended_for_contract=False; eos_deferred=False; last_gap_signature=(); stopped_on_eos=False; eos_id=int(model.tokenizer.eos) if model.tokenizer.eos is not None else None
        while ordinal < current_limit:
            ordinal += 1
            if is_cancelled and is_cancelled(): raise base.R39InferenceError("REQUEST_CANCELLED","Generation was cancelled.")
            seed = hash(request_id) ^ (state.pos * 0x9E3779B9); next_token = base._sample(logits,recent[-64:],temperature,top_p,50,1.05,seed)
            if eos_id is not None and next_token == eos_id:
                partial="".join(response_parts); gaps=_response_contract_gaps(partial,contract)
                if gaps and ordinal < _ABSOLUTE_RESPONSE_HARD_CAP:
                    masked=logits.copy(); masked[eos_id]=np.float32(-np.inf); next_token=base._sample(masked,recent[-64:],temperature,top_p,50,1.05,seed ^ 0x5F3759DF); signature=tuple(gaps)
                    if signature != last_gap_signature: last_gap_signature=signature; yield {"type":"STATUS","phase":"RESPONSE_CONTRACT","reason":"EOS deferred because requested or verified response parts remain incomplete/inconsistent: " + ", ".join(gaps) + "."}
                    eos_deferred=True
                    if next_token == eos_id: stopped_on_eos=True; break
                else: stopped_on_eos=True; break
            text=decoder.push(next_token)
            if first_ms is None: first_ms=int((time.monotonic()-started)*1000)
            if text: response_parts.append(text); yield {"type":"DELTA","phase":"GENERATING","text":text,"firstDeltaLatencyMs":first_ms}
            generated_tokens.append(next_token); recent.append(next_token); decode_started=time.monotonic(); logits=_forward_hot(model,next_token,state,need_logits=True); assert logits is not None
            if not wrap_notified and ordinal >= int(budget["wrapAt"]): wrap_notified=True; yield {"type":"STATUS","phase":"RESPONSE_BUDGET","reason":f"Soft wrap point reached at token {ordinal}; generation may continue until the requested response is complete."}
            if ordinal % _LIVE_CHECKPOINT_EVERY == 0 and response_parts: _publish_generated_checkpoint(model,thread_id,prompt,tokens,"".join(response_parts),list(generated_tokens),state,logits,kind=f"assistant-live-{ordinal}")
            if ordinal <= 4 or ordinal % 32 == 0: yield {"type":"STATUS","phase":"DECODE_PROGRESS","reason":f"Decode step {ordinal} · {time.monotonic()-decode_started:.3f}s · backend={('native' if native else 'python')}."}
            if ordinal >= current_limit and current_limit < _ABSOLUTE_RESPONSE_HARD_CAP:
                partial="".join(response_parts); gaps=_response_contract_gaps(partial,contract); structural=_looks_structurally_open(partial)
                if structural or gaps:
                    next_limit=min(_ABSOLUTE_RESPONSE_HARD_CAP,current_limit+_RESPONSE_EXTENSION_STEP)
                    if next_limit > current_limit:
                        current_limit=next_limit; extended_for_structure=extended_for_structure or structural; extended_for_contract=extended_for_contract or bool(gaps); reason="open code/syntax structure" if structural and not gaps else "unfinished/inconsistent requested response parts" if gaps and not structural else "open structure and unfinished/inconsistent requested parts"; yield {"type":"STATUS","phase":"RESPONSE_BUDGET","reason":f"Completion headroom extended to {current_limit} token(s) because of {reason}."}
        tail=decoder.finish()
        if tail: response_parts.append(tail); yield {"type":"DELTA","phase":"GENERATING","text":tail,"firstDeltaLatencyMs":first_ms}
        response_text="".join(response_parts); terminal_published=False
        if generated_tokens and logits is not None: terminal_published=_publish_generated_checkpoint(model,thread_id,prompt,tokens,response_text,generated_tokens,state,logits,kind="assistant-terminal-open")
        if generated_tokens: _start_speculative_warmup(model,thread_id,prompt,list(tokens),_clone_state(state),list(generated_tokens),response_text)
        completion_bits=["Local R39 generation completed"]
        if terminal_published: completion_bits.append("response-state checkpoint published")
        completion_bits.append("next-turn warmup continues opportunistically")
        if eos_deferred: completion_bits.append("semantic completion gate deferred premature EOS")
        if extended_for_structure: completion_bits.append("completion headroom was used for open code/syntax")
        if extended_for_contract: completion_bits.append("completion headroom was used for requested/verified response parts")
        final_gaps=_response_contract_gaps(response_text,contract)
        if final_gaps: completion_bits.append("semantic verification warnings: " + ", ".join(final_gaps))
        elif contract.get("coding"): completion_bits.append("coding response consistency checks passed")
        if not stopped_on_eos and ordinal >= current_limit: completion_bits.append(f"generation reached safety ceiling {current_limit}")
        ...
        event_type="CANCELLED" if exc.code=="REQUEST_CANCELLED" else "FAILED"; phase="CANCELLED" if event_type=="CANCELLED" else "ERROR"; yield {"type":event_type,"phase":phase,"reason":exc.detail,"categories":[exc.code],"totalLatencyMs":int((time.monotonic()-started)*1000)}
...
def _cleanup_jobs(now: float | None = None) -> None:
    ...
    with _JOB_LOCK:
        ...
        for rid in expired: _JOBS.pop(rid,None)
        if len(_JOBS)>_MAX_JOBS:
            ordered=sorted(_JOBS.items(),key=lambda item:float(item[1].get("updatedAt",0)))
            for rid,job in ordered:
                if len(_JOBS)<=_MAX_JOBS: break
                if job.get("done"): _JOBS.pop(rid,None)


def _append_job_event(job: dict[str, Any], event: dict[str, Any]) -> None:
    condition=job["condition"]
    with condition:
        job["events"].append(dict(event)); job["updatedAt"]=time.time(); condition.notify_all()


def _run_job(job: dict[str, Any], payload: dict[str, Any], is_cancelled) -> None:
    try:
        for event in _generate_hot_events(payload,is_cancelled): _append_job_event(job,event)
    except Exception as exc:
        ...
        condition=job["condition"]
        with condition: job["done"]=True; job["updatedAt"]=time.time(); condition.notify_all()
...
def _job_for(payload: dict[str, Any], is_cancelled):
    ...
    with _JOB_LOCK:
        ...
        condition=threading.Condition(threading.RLock()); job={"requestId":request_id,"createdAt":time.time(),"updatedAt":time.time(),"events":[],"done":False,"condition":condition}; worker=threading.Thread(target=_run_job,args=(job,dict(payload),is_cancelled),name=f"swrlz-r39-job-{request_id[-16:]}",daemon=True); job["thread"]=worker; _JOBS[request_id]=job; worker.start(); return job


def _job_snapshot() -> list[dict[str, Any]]:
    _cleanup_jobs(); now=time.time()
    with _JOB_LOCK:
...
def inspect_engine():
    try:
        model=_get_model(); jobs=_job_snapshot(); contexts=_context_snapshot(); native=native_bridge.available()
        return {"ok":True,"oneTokenReady":True,"interactiveReady":True,"engineId":ENGINE_ID,"modelId":model.manifest.get("modelId","R39"),"modelSha256":MODEL_SHA256,"tocCount":model.header["tocCount"],"tensorCount":len(model.desc),"tokenCount":len(model.tokenizer.tokens),"graphNodeCount":len(model.graph.get("nodes",[])),"hotServerVersion":HOT_SERVER_VERSION,"hotRevision":HOT_REVISION,"nativeBackendAvailable":native,"nativeDirectQuantizedMatvec":native,"pythonReferenceFallback":True,"warmModelResident":True,"warmModelAgeSeconds":round(max(0.0,time.time()-_MODEL_READY_AT),2),"decodedCacheBytes":int(getattr(model,"_hot_tensor_cache_bytes",0)),"decodedCacheBudgetBytes":_CACHE_BUDGET_BYTES,"tokenEmbeddingRowCacheEntries":len(getattr(model,"_hot_token_row_cache",{})),"prefillSkipsIntermediateLogits":True,"incrementalConversationPrefill":True,"multiCheckpointConversationPrefill":True,"progressivePrefillCheckpoints":True,"progressivePrefillCheckpointEveryTokens":_PREFILL_CHECKPOINT_EVERY,"generationPublishesReusableCheckpoints":True,"terminalAssistantCheckpointBeforeComplete":True,"speculativeNextTurnWarmup":True,"nextRequestNeverWaitsForWarmup":True,"liveCheckpointEveryGeneratedTokens":_LIVE_CHECKPOINT_EVERY,"prefillStaticTensorFastPath":True,"adaptiveResponseBudget":True,"softWrapAdvisory":True,"structureAwareBudgetOverrun":True,"responseCompletionContract":True,"codingConsistencyGuidance":True,"codingSemanticChecks":True,"eosDefersForMissingRequirements":True,"stableResponseDirectiveForCacheReuse":True,"vectorizedGroupedAttention":True,"batchedMatmulAttention":True,"absoluteResponseHardCap":_ABSOLUTE_RESPONSE_HARD_CAP,"conversationContextCacheMaxTokens":_CONTEXT_MAX_TOKENS,"conversationContextCacheTtlSeconds":_CONTEXT_TTL_SECONDS,"conversationContextMaxCheckpoints":_CONTEXT_MAX_CHECKPOINTS,"conversationContextCaches":contexts,"connectionDiagnostics":True,"detachedGeneration":True,"replayableJobs":jobs,"activeDetachedJobs":sum(1 for job in jobs if not job["done"])}
    except base.R39InferenceError as exc:
...
def generate_events(payload, is_cancelled=None):
    ...
    if job is None:
        yield from _generate_hot_events(payload,is_cancelled); return
    index=0; condition=job["condition"]
    while True:
        event=None
        with condition:
            while index>=len(job["events"]) and not job.get("done"): condition.wait(timeout=1.0)
            if index<len(job["events"]): event=dict(job["events"][index]); index+=1
            elif job.get("done"): break
