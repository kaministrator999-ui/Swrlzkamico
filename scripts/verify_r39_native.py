...
def main() -> None:
    ...
    for kind, cols, rows in cases:
        ...
        # Keep half scales finite for quantized formats.
        if kind in {"q4_0", "q8_0"}:
            stride = 18 if kind == "q4_0" else 34
            for off in range(0, raw.size, stride):
                raw[off:off+2] = np.frombuffer(np.float16(0.03125).tobytes(), dtype=np.uint8)
        elif kind == "q4_k":
            for off in range(0, raw.size, 144):
                raw[off:off+2] = np.frombuffer(np.float16(0.03125).tobytes(), dtype=np.uint8)
                raw[off+2:off+4] = np.frombuffer(np.float16(0.015625).tobytes(), dtype=np.uint8)
        elif kind == "q6_k":
            for off in range(0, raw.size, 210):
                raw[off+208:off+210] = np.frombuffer(np.float16(0.03125).tobytes(), dtype=np.uint8)
        ...
        tol = 2e-3 + 2e-4 * float(np.max(np.abs(expected)))
        if not np.allclose(expected, actual, rtol=2e-4, atol=2e-3):
            raise AssertionError(f"{kind} mismatch: max_abs_error={err} tolerance~{tol}")
